...
对每个输入文件，主流程最终只生成两张图片：

- `*_projection.png`：二维正投影图
- `*_density.png`：灰度密度图

交互模式会额外在 `_interactive_preview/` 下生成预览图片，便于边调角度边观察结果。
...
- `cli.py` 负责主流程命令行入口，组织输入遍历、预处理、投影和交互调参
...
### 6. 如果我只想做 `stl -> ply` 转换，不想跑完整投影流程怎么办？
